...
- Uses an invalidated or failed authorization to avoid required
...
- Every forwarded request MUST be covered by a valid authorization from fully
  verified attestation for its provider, model or route, cryptographic identity, and key epoch.
...
- Trace acquisition separately from use: eviction must prevent subsequent
  ...
  may continue within its caller deadline. Do not
  request cancellation of unrelated HTTP/2 streams merely because of eviction.
- Check that the report, authenticated encryption key, and transport
  identity are published as one authorization. Flag independent
  pin caches or partial publication that can give these values different lifetimes.
- Fully verify uncached TLS-binding authorization. Reuse it while its attested
  identity and required keys remain usable, until explicit invalidation, eviction,
  or process exit. Flag evidence-expiration renewal or connection-loss invalidation.
  Recheck verified NRAS time eligibility only before initial publication. Caller
  deadlines still bound waiting, streaming, and downstream writes. Normal and
  offline reuse follow the same rule with their existing admission checks.
- Approval withdrawal requires an updated package or admission policy and restart
  of all affected instances; restarting with unchanged policy can reapprove the
  withdrawn state. See the transport reference for operational responsibility.
...
- For E2EE/router providers, identify the backend model endpoint and E2EE key
  ...
  and E2EE requirements when both apply; invalidate and re-attest on the precise trust/key failures in the retry contract.
...
- Publishes shared verification results without rechecking initial NRAS admission time eligibility and
...
- Require concurrent unit and integration coverage for shared-state changes,
  ...
  replacement, admission time checks, and invalidation interleavings, not just simultaneous
