...
func TestTestE2EEChutes_MissingInstanceID(t *testing.T) {
	raw := &attestation.RawAttestation{
		SigningKey: "dGVzdA==", // any non-empty value
		InstanceID: "",
...
func TestDoE2EEChutesStreamTest(t *testing.T) {
	...
	// Derive stream key using HKDF-SHA256 (same as server).
...
func TestDoE2EEChutesStreamTest_NoE2EInit(t *testing.T) {
	// Server sends e2e events without e2e_init → must fail.
...
func TestDoE2EEChutesStreamTest_E2EError(t *testing.T) {
	// Server sends an e2e_error event.
...
// deriveStreamKeyForTest derives a Chutes stream key using HKDF-SHA256 for test server simulation.
...
// encryptChunkForTest encrypts a stream chunk with ChaCha20-Poly1305 for test server simulation.
