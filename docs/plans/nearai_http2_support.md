...
The `tinfoil_v3_cloud` authorization cache shares model-independent router
evidence by provider and
...
## Review Baseline and Commit Sequence

Review the changes relative to `origin/main`. The historical survey describes
that baseline, not an intermediate revision of this branch. The initial commit,
`Add plan for NearAI http/2 support`, contains this plan; the numbered phases
below correspond to the subsequent commits in order. Use
`git log --reverse --oneline origin/main..HEAD` and inspect each commit against
its parent. The detailed test plan describes the completed suite; the phase
sections identify when behavior and coverage are introduced.

At the baseline, the proxy report/signing-key caches and NEAR SPKI cache use
`AttestationCacheTTL` (one hour). This change replaces those caches for
TLS-binding providers with an authorization store that has no expiration from
its introduction in phase 5, then activates it for Tinfoil in phase 6 and NEAR
in phase 7. Discovery, model-list, negative, and non-TLS-binding caches retain
their separate policies. The baseline has no evidence-derived authorization
expiry field or inference deadline helper to remove.

The official upstream `go-tdx-guest` dependency and version are unchanged from
`origin/main`; this work does not change `go.mod` or `go.sum`. Existing TDX,
SEV-SNP, and NVIDIA certificate verification remains an admission check.
NRAS already requires `exp`; phase 5 adds a 10-second JWT validation leeway and
transient authenticated admission bounds. Phases 6 and 7 pass those bounds to
the final verification/publication check. They are never cached as a reuse
limit. Phase 5 tests signed-JWT publication and later reuse; phase 7 tests
proxy reuse after connection loss in normal and offline operation.
...
The lifetime model assumes that these providers create TLS and E2EE keys
during image boot and retain private keys only in that measured environment. A repository or discovery metadata change
...
cached authorization. Fully verify an uncached authorization. Reuse it while
its attested identity and required encryption keys remain usable, until explicit
invalidation, eviction, or process exit. Evidence expiration alone does not
trigger renewal. Loss of all inference connections also does not trigger
re-attestation. An authorization miss for a new model backend requires
full verification; model-independent router evidence remains shared within its
attested scope.
...
- preserve fail-closed admission verification and E2EE authentication for every
...
change: TLS-binding authorizations have no wall-clock reuse deadline. Fresh
client nonces and, online, verified NRAS JWT time claims establish freshness
when authorization is created. TDX verification uses the pinned upstream
`github.com/google/go-tdx-guest` module. Review and test this lifetime policy
separately. Preserve
admission-time cryptographic verification, measurement policy, factor
enforcement, endpoint support, encryption algorithms, and response schemas.
Global redirect enforcement is a separate implementation phase because
...
## Live Investigation Results (Historical)
...
These results showed that the services supported the proposed transport model
at the time of the investigation. They do not replace automated tests because endpoint configuration and
...
9. **Fail closed on missing state.** A TLS-binding provider with an empty,
   malformed, or mismatched transport identity fails before publication and
   before inference I/O. This is an authorization-construction error, not a
   factor failure that `allow_fail` can bypass. Cached authorization has no
   evidence-derived expiration; each new TLS handshake still enforces current
   WebPKI, TLS 1.3, CT, and SPKI validation.
...
16. **No ambiguous NEAR discovery data.** Keep neardirect discovery validation
   ...
   parser for discovery. The strict key-rejection decoder in phase 6 has a
   separate scope: an ambiguous error must not authorize inference replay. Reject the complete refresh on malformed or
...
This model assumes that providers create TLS and E2EE keys during image boot
and do not transfer their private keys to another image. Attestation binds the
public keys to the measured image; it does not prove future private-key custody. Full verification establishes an initial authorization for
that identity and the applicable model or router scope. Reuse depends on the
attested keys, not on evidence age or connection lifetime. Keep authorization
until a classified trust/key failure, explicit invalidation, capacity eviction,
or process exit. Do not add an authorization TTL, connection-count condition,
background renewal, or periodic vendor reappraisal.

Select the effective supply-chain repository from the same discovery snapshot
as the authority on an authorization miss. Do not include repository metadata,
discovery refresh time, or resolver generation in the authorization cache key.
A required discovery refresh must still fail closed and a changed route must
select its own authorization. Metadata changes alone do not invalidate an
existing authorization for unchanged keys and scope.

The immutable entry contains the report, E2EE key when required, authority,
SPKI, and opaque generation. A model-specific backend key that is absent from
the cache requires full verification; never forward an unverified request or
borrow another model's key. A cloud router whose attested key legitimately
covers multiple models may share that authorization. A new model name alone
does not force another attestation of model-independent router evidence.
...
- Cancellation, request deadlines, DNS/dial failures, connection closure, EOF,
  ...
  themselves establish key failure. Fail the affected request as appropriate
  and retain authorization. Pre-connection retries remain limited by the retry
  contract; all other ambiguous outcomes forbid application replay.

A failed request must never delete or alter a replacement generation. Preserve
clear, non-secret failure diagnostics at the request boundary. Do not add a
persistent key-failure marker or negative-cache entry for ordinary I/O errors.
Closing every inference connection, including idle-timeout cleanup, does not
invalidate authorization: new connections must authenticate the cached SPKI
before sending inference bytes. Connection reuse itself is not proof that an
E2EE backend remains valid.

### Evidence freshness at authorization creation

Use a new client-generated cryptographic nonce for each full attestation and
verify its binding through the applicable quote, REPORTDATA, and GPU evidence.
Concurrent callers may join that one bounded verification operation; they do
not share inference encryption sessions. Online NRAS verification additionally
requires a correctly signed overall JWT with valid time claims. These checks
establish freshness at admission; none creates a continuing authorization
expiration.

| Evidence or state | Admission-time behavior | Cached authorization behavior |
| --- | --- | --- |
| TDX or SEV-SNP quote/report and REPORTDATA key binding | Verify structure, signatures and certificate chains as supported by the selected mode, client nonce, measurements, and required key binding. No quote-age TTL. | Retain the resulting key authorization without a wall-clock expiration. |
| Intel online collateral | Preserve upstream collateral, revocation, and certificate validity checks during verification. Certificate and collateral dates are checked by the verifier at admission; they do not supply a cached authorization deadline. | Later collateral or certificate expiration does not cause renewal. |
| NVIDIA NRAS overall JWT | Verify signature, required `exp`, and applicable `nbf`/other time claims with the 10-second leeway introduced in phase 5. Per-GPU diagnostic JWTs do not establish independent trust or freshness. | Do not use `exp` as an authorization or inference deadline. |
| Online SEV-SNP certificate verification | Preserve verification against trusted AMD roots and the verifier's certificate-time checks. | Do not derive a reuse deadline from VCEK/VLEK or issuer dates. |
| Local NVIDIA SPDM/EAT verification | Preserve GPU certificate-chain, certificate-time, SPDM-signature, and nonce checks. | Do not derive a reuse deadline from GPU certificate dates. |
| Proof of Cloud JWT | Preserve the existing policy and claims checks. The current path does not independently verify the JWT signature; do not describe its dates as authenticated freshness. | No authorization expiry. |
| Sigstore/Fulcio and release evidence | Preserve supply-chain verification and validation at the applicable authenticated signing time. | Historical signer-certificate dates do not expire authorization. |
| Inference TLS certificate | Every new handshake must pass current WebPKI, TLS 1.3, CT, and cached SPKI authentication. | Certificate dates do not set a cached authorization deadline or end existing streams. |
| Discovery, repository metadata, cache age, idle timeout, JWKS refresh | Preserve routing and resource-management checks. | No authorization expiry or connection-loss invalidation. |

Malformed or unauthenticated evidence, invalid required time claims, and
certificates rejected by the verifier must retain their factor failure behavior.
Keep explicit `allow_fail`, offline, and debug-force semantics; never relabel an
allowed failure as successful verification. In particular, an expired NRAS JWT
must not pass its factor. Recheck a successfully verified NRAS JWT's time
eligibility immediately before publishing initial authorization if other
verification work delayed publication. Keep any date needed for this check in
the bounded verification operation, not as a cache or request deadline. Report
promotion is not a new admission and does not repeat this check. Fresh nonce
binding and NRAS JWT validity are the attestation freshness checks; existing
certificate/collateral validity checks remain part of cryptographic admission,
not additional attestation-age or reuse timers.

Normal and `--offline` modes use the same authorization reuse policy. Offline
mode retains its documented omissions of online verification, including NRAS
and Intel collateral, and supplies no replacement freshness TTL. Local nonce,
measurement, key-binding, and supported cryptographic checks still run. Offline
mode is not network-free: provider discovery and attestation retrieval remain
necessary on cache misses. Reports must distinguish checks performed at
admission from skipped checks and must not imply continuing vendor approval.

### TDX admission verification

TDX verification uses the pinned upstream `github.com/google/go-tdx-guest`
module. The verifier checks quote signatures, certificate chains, and applicable
online collateral and revocation status at admission. Client nonce and required
REPORTDATA binding checks remain part of authorization creation. Backend and
gateway TDX results contribute no cached authorization expiration.

Certificate and collateral dates are evaluated within upstream verification;
teep does not parse them again to create a reuse deadline. Retain diagnostic
dates where useful and keep the NRAS admission-time eligibility check within
the bounded verification operation. Neither supplies an inference deadline
under the key-based lifetime policy.

### Approval withdrawal and operating assumptions

Approval withdrawal is an operational policy update followed by restart of all
affected teep instances. Teep maintainers may publish a security advisory that
package maintainers or a future teep updater distribute with the changed policy
or package. Operators must apply the withdrawal and restart; a restart without
a changed policy merely permits the same deployment to pass again. Remove an
unacceptable measurement, tighten an expressible platform requirement, or
disable the affected provider when the current policy cannot express the
restriction. Restart discards cached authorizations and connections, so new
requests must pass the updated admission policy.

Document this rule and its operational responsibility now. Do not implement an
advisory feed, updater, automatic policy reload, administrative invalidation
API, or new denylist format in this change. Existing internal invalidation and
shutdown remain available for their current purposes. External audits are
expected to inform withdrawal decisions covering application dependencies and
platform/GPU security requirements, not only CVM image hashes.

This model deliberately accepts that a working or stolen key can retain a
historical approval until invalidation or restart. It does not provide periodic
vendor revocation discovery. An instance that does not receive and apply an
advisory can continue using withdrawn approval indefinitely. Immutable image
identity does not by itself prove current platform security. State these limits
in the maintained documentation rather than claiming equivalence to periodic
attestation or NRAS reassessment.
...
is still the current entry, then return the immutable snapshot.
...
be used directly. If the entry is absent or invalidated, initiate or join full
...
Each attempt remains bounded by the logical request deadline. Apply that
deadline to connection waits, response processing, and downstream writes;
response writers must support `SetWriteDeadline`, directly or through `Unwrap`,
and failure to apply it must fail the request. Do not attach certificate,
collateral, or JWT expiration to an inference context. Evidence expiring during
an acquired attempt does not cancel it or forbid report promotion. Do not
extend the logical deadline on retries. Ordinary deletion after acquisition
does not add a per-request validity poll or cancel another client's streams.
...
`Authority()`. Its `SupplyChainRepo()` returns the repository selected from the
...
inference URL. This avoids a route-selection race where a discovery refresh
...
- a server-owned opaque authorization generation assigned when the entry is
  published and used only for equality checks.

The entry has no wall-clock expiration or evidence-derived reuse deadline.

The entry must not contain `RawAttestation`, a raw response body, an attestation
challenge nonce, an E2EE session, or request-specific material. After shared
verification completes, each waiter must acquire the published
entry again so invalidation before delivery cannot authorize an attempt.
Blocked verification returns its diagnostic report without publishing an
authorization. Every joined caller creates its own
...
opaque equality tokens. Store it on the server-owned authorization store; do
not add writable package state.
...
cache capacity. Do not use a local TTL or evidence-derived expiration for
TLS-binding authorization. JWT, certificate, and collateral dates apply during
admission as described above, not on cache hits. Do not add expiry conditions
for derived identity, repository metadata, connection loss or idle time, or
E2EE key material in the same authorization.
...
Use `prepareAuthorizedRequest` and `provider.PrepareInference` to consume
the signing key from the immutable authorization snapshot. On an authorization
hit, do not read a second key cache. Keep `buildUpstreamBody` for the unchanged
non-TLS-binding path. A snapshot cannot lack required key material after
...
`nearcloud` and `neardirect`. In `internal/verify/verify.go` and
`internal/verify/tls_inference.go`, resolve a dynamic route once before
attestation and retain that immutable value through E2EE
...
Standalone verification performs fresh admission checks, including nonce and
online NRAS JWT validity. Once admitted, its inference attempt uses the logical
request deadline without an evidence-expiration deadline. Proxy cache hits do
not repeat admission-time freshness checks.
...
Commit: `Add attested transport identities and bounded HTTP transport construction`.

Primary files (including focused tests):

- `internal/attestation/attestation.go` and `report.go`, with identity/report tests
- `internal/provider/tinfoil/attester.go`
- `internal/config/config.go`
- `internal/proxy/pinned_upstream.go` and `proxy.go`
- `internal/tlsct/identity.go`, `pooled.go`, `transport.go`, and `pinned.go`, with tests
...
Commit: `Reject outbound redirects and block inference redirect responses`.
...
Commit: `Bind NEAR attestation evidence to the live TLS peer`.

Primary files (including focused tests):

- `internal/provider/neardirect/nearai.go` and `nearai_test.go`
- `internal/provider/nearcloud/nearcloud.go`
- both NEAR `transport_binding_test.go` and `pinned_test.go` files
- `cmd/teep/main_test.go` and `reverify_tls_test.go`
...
Commit: `Add immutable request routes and reject ambiguous NEAR discovery`.

Primary files (including focused tests):

- `internal/provider/provider.go`, `route.go`, and `route_test.go`
- `internal/provider/neardirect/nearai.go` and `endpoints.go`, with discovery and refresh tests
- `internal/provider/tinfoil/attester.go` and `resolver.go`, with route and refresh tests
- `internal/proxy/route.go` and `route_internal_test.go`
- `internal/tlsct/identity.go` and `identity_test.go`
...
4. Add key derivation and test route snapshots and request-scoped resolution.
   Defer shared inference operations, explore report handoff, and production
   cache consumers to phase 6; NEAR caller activation follows in phase 7.
...
Commit: `Add atomic authorization cache and authenticated evidence validity`.

Primary files (including focused tests):

- `internal/proxy/authorization.go` and authorization cache tests
- `internal/attestation/admission.go`, `nvidia.go`, and `nvidia_eat.go`
- `internal/attestation/sev.go` and `sev_admission_test.go`
- `internal/attestation/tdx_admission_test.go` and `nvidia_leeway_test.go`
- `internal/proxy/authorization_admission_test.go`

Work:

1. Add the immutable authorization entry with report, minimum verified E2EE
   public key, transport identity, and opaque generation. Do not store an
...
5. Match the existing cache capacity and give the new store no wall-clock
   authorization expiry from construction.
   Implement the admission-time freshness contract, including the transient
   NRAS publication check. Use upstream TDX admission verification without
   deriving a reuse deadline from its certificates or collateral.
6. Add deterministic tests for atomic publication, delayed singleflight delivery,
   invalidation before acquisition, admission-time freshness failure, shutdown,
   stale-generation updates, age-independent reuse, and verification concurrency
   limits. Add upstream TDX, SEV-SNP, and NRAS admission regression tests.
   Include real signed NRAS JWT publication/reuse tests in this phase. The
   cache-hit test advances past evidence validity and checks concurrent reuse
   without verification. Phase 7 adds normal/offline reconnect coverage through
   the activated NEAR proxy path.
...
Commit: `Use atomic route authorizations for Tinfoil inference`.

Primary files (including focused tests):

- `internal/proxy/authorized_*.go`, `proxy.go`, `explore.go`, `dashboard.go`, and related tests
- `internal/provider/route.go`, `key_rejection.go`, and Tinfoil route/attester code
- `internal/verify/verify.go`, `tls_inference.go`, `e2ee.go`, and capture attempt tests
- `internal/tlsct/inference_retry.go` and `checker.go`
- `internal/jsonstrict/unique.go` and `unmarshal.go`, with rejection parser tests
- `internal/capture/capture.go`, `internal/config/config.go`, and provider fetch error paths
- `internal/e2ee/ehbp.go` and `ehbp_test.go`
...
7. Share Tinfoil cloud router evidence by provider and authority while retaining
   bounded per-model report views and model-specific outcomes.
8. Pass authenticated NRAS admission bounds from `fetchVerified` to publication;
   check them at the end of standalone verification too. Preserve cancellation
   causes, release capture/client resources, and let callers log parser failures
   with provider, model, authority, and a specific non-secret reason.
9. Reject duplicate JSON members in candidate key-rejection envelopes through
   the shared strict decoder. Ambiguous responses cannot trigger recovery.
10. Return the actual request outcome and accumulated phase durations, including
...
11. Reject an empty EHBP response and a partial frame-length prefix. Keep the
    reader regression tests with this change.
12. Remove EHBP response handling from the generic relay when Tinfoil moves to
    authorized inference. This keeps the generic relay below the complexity
    limit before the NEAR migration. Phase 8 removes the remaining obsolete
    helpers and their tests with the manual transport implementation.
...
Commit: `Move NEAR inference to attested HTTP/2 transport pools`.

Primary files (including focused tests):

- `internal/provider/inference.go`, `key_rejection.go`, and NEAR provider code, with tests
- `internal/proxy/authorized_inference.go`, `proxy.go`, and `pinned_upstream.go`
- proxy NEAR authorization, capacity, route diagnostics, and E2EE tests
- `internal/verify/factory.go`, `tls_inference.go`, `verify.go`, and `e2ee.go`, with tests
- `internal/tlsct/connection_budget.go`, `pooled.go`, `checker.go`, and capacity tests
- `internal/config/config.go` and `internal/attestation` collateral retrieval helpers/tests
- `internal/e2ee/session.go`, `relay.go`, and reassembly regression tests
- docs/api_support.md
...
10. Share request preparation between proxy and standalone inference: invoke
    ...
    headers, and let net/http select body framing. Preserve caller deadlines
    and share session cleanup. The proxy uses per-attempt cancellation for
    resource cleanup; evidence dates never add an inference deadline. Keep
    report and retry orchestration in their respective callers.

11. Bound physical sockets, including concurrent HTTP/2 dialing, and return
    typed capacity errors without retry or negative caching. Preserve Intel PCS
    and AMD KDS retrieval causes through verification results.
12. Centralize collateral retries in `config.NewAttestationClient`; getters must
    not multiply that loop. Permit at most three eligible attempts under one
    timeout and verification deadline; local socket capacity does not retry.
    Construct the proxy attestation client through this same constructor and
    test that the proxy performs only that bounded retrieval sequence.
13. Decrypt each NEAR reassembly delta once and use that result for content and
    tool-call metadata. Test mandatory session headers and request-route
    diagnostics through the activated path.
14. Authenticate SSE completion through the end of the underlying response,
    including EHBP frames after the application end marker. Reject incomplete
    NEAR streams and explicit SSE errors, and bound non-streaming reassembly.
    Keep shared SSE parsing, proxy/standalone completion, and failure-retention
...
16. Test connection-loss reuse in normal and offline operation, missing model
    ...
    local HTTP/2 protocol, replay, connection reuse, and stream-lifetime tests.
...
Commit: `Remove manual NEAR HTTP handlers and independent SPKI cache`.

Primary files (including focused tests):

- remove both NEAR provider `pinned.go` and `pinned_test.go` files
- remove `internal/attestation/spki.go` and `spki_test.go`
- `internal/provider/provider.go` and NEAR provider declarations
- `internal/proxy/proxy.go`, `pinned_upstream.go`, and `explore.go`
- proxy authorization, generic encryption, redirect, and NEAR E2EE tests
- rename the NEAR test upstream helper to `mock_near_upstream_test.go`
- `internal/proxy/dashboard.go` and `templates/dashboard.html`
...
1. Make a test-migration inventory that maps every test in the two removed
   ...
   behavior.
2. Remove the manual interface and dispatch code. Remove the remaining generic
   EHBP helpers, request-building state, and obsolete helper tests after phase 6
   removes their response-dispatch callers.
...
5. Remove obsolete connection-close assertions and add no-`Connection` and
   HTTP/2 assertions.
6. Remove unused SPKI-cache state and dashboard counters after confirming no
   callers remain. Preserve the non-TLS-binding report/signing-key cache policy.

Commit purpose: leave one transport implementation and no HTTP/1.1 connection
writer in the NEAR providers.

### Phase 9: Validate transport and authorization behavior

Commit: `Validate attested HTTP transport behavior across Go versions`.

Primary files: `.github/workflows/ci.yml`, `Makefile`, and selected live proxy
integration tests. Local transport, response-processing, and authorization
regressions accompany their implementation in phases 5 through 7.

1. Run the local HTTP/2, authorization, response-completion, and concurrency
   suites from the implementation phases across the supported Go versions.
   This phase adds no runtime Go implementation changes.
2. Add and run selected live reuse, concurrent-provider, and key-recovery tests
   and report targets for `tinfoil_v3_cloud`, `nearcloud`, and `neardirect` only.
   Keep direct Tinfoil coverage local/fixture-based for this review.
3. Update immediate API, provider, README, and security-rule descriptions affected
   by this validation. The following commits consolidate maintained references.
   Audit prompts and reports are excluded from this review and are not current
   design authorities.

### Phase 10: Document HTTP and TLS transport contracts

Commit: `Document HTTP and TLS transport contracts`.

Update `docs/transport/README.md`, `retries.md`, `redirects.md`, and `testing.md`
with the completed contracts and regression references. Align `AGENTS.md`,
`README.md`, `README_ADVANCED.md`, `docs/api_support.md`, Tinfoil provider
support, and the related PhalaCloud/Tinfoil plan with those contracts.

State admission-time verification, key-based authorization reuse, gateway versus
backend identity scope, and advisory-driven withdrawal through policy update
and restart of every running instance. Evidence expiration and loss of all
connections do not trigger renewal. No updater, advisory feed, or administrative
invalidation API is implemented. Link shared rules instead of duplicating them.

### Phase 11: Require documentation maintenance

Commit: `Add AGENTS.md block about keeping docs in sync`.

Add the documentation-maintenance requirements to `AGENTS.md`, including
transport references, provider guarantees, plans, and review instructions.

### Phase 12: Align code review instructions

Commit: `Update code review instructions`.

Update `.github/instructions/code-review.instructions.md` to match the
implemented authorization lifetime, concurrency, failure classification,
cryptographic enforcement, and documentation rules in `AGENTS.md`.

### Phase 13: Remove redundant instructions

Commit: `Remove redundant CLAUDE.md`.

Remove the redundant `CLAUDE.md` entry; maintain the repository instructions in
`AGENTS.md`.
...
- concurrent joined NEAR and tinfoil requests use distinct E2EE sessions and
  ...
  challenge nonces;
- invalid NRAS time claims fail admission, including expiration before initial
  publication; later JWT, certificate, and collateral expiration does not cause
  re-attestation, reject a cache hit, or cancel connection waits and streams;
...
- admission tests distinguish required NRAS dates, malformed/expired JWTs,
  failed signatures, the 10-second leeway, offline and allowed failures, and
  historical signing validity; no cache reparses raw evidence for dates;
- normal and offline cache hits have no wall-clock expiry, and closing all
  connections preserves authorization for a newly SPKI-authenticated connection;
...
- a stale report clone cannot overwrite a newer report while marking
  `e2ee_usable`;
- report promotion preserves identity and generation and never creates an
  expiration or repeats admission-time freshness checks;
...
- concurrent cache hits, invalidation, re-attestation, cancellation, and SPKI
...
Test reuse after evidence expiration separately from admission: an expired
NRAS JWT cannot pass initial verification, but its later expiration does not
revoke admitted keys. Never accept a stale route after a required discovery
refresh fails. All refresh and mismatch errors must block the affected request. A valid
...
been removed. Supply credentials through the environment or supported `.env`
loading without printing them. Integration Makefile targets enable the test
credential loader; do not execute the credential file as a shell script.
...
```sh
go test -race -short ./internal/tlsct
go test -race -short ./internal/provider/tinfoil
go test -race -short ./internal/provider/neardirect
go test -race -short ./internal/provider/nearcloud
go test -race -short ./internal/proxy
go test -race -short ./internal/integration -run '^TestIntegration_(NearCloud|NearDirect|Tinfoil)_Fixture$'
```

Search for obsolete behavior before final validation:

```sh
rg -n 'Connection.*(close|keep-alive)|ConnClosingReader|WriteHTTPRequest|PinnedHandler' internal docs/transport docs/providers docs/api_support.md README.md README_ADVANCED.md
...
```sh
make check
make integration-tinfoil integration-neardirect integration-nearcloud
make integration-concurrent-providers
make integration-tinfoil-fixture integration-neardirect-fixture integration-nearcloud-fixture
make report-tinfoil report-neardirect report-nearcloud
git diff --check
```

Live integration and reports in this review are restricted to
`tinfoil_v3_cloud`, `nearcloud`, and `neardirect`; do not run other provider
aliases. Unit coverage still covers shared behavior and unaffected providers.
...
- each TLS-binding authorization atomically contains a report, minimum E2EE
  public key, transport authority, fingerprint, and opaque generation, without
  an expiry, `RawAttestation`, or request-specific state;
...
- acquisition rejects invalidated generations, explicit invalidation
...
- non-TLS-binding providers retain their existing cache and per-request E2EE
  material behavior;
- `make check`, local regression tests, and `git diff --check` pass without
  new exemptions; selected live integration/report outcomes are recorded, and
  external verification failures remain fail-closed;
- the pinned upstream go-tdx-guest module passes TDX admission regression
  tests, and backend/gateway TDX evidence supplies no cached reuse deadline;
- admission-time nonce and NRAS time checks remain enforced as applicable,
  while normal and offline reuse has no evidence or connection-lifetime timer;
- documentation states advisory-driven withdrawal through policy update and
  restart, with no updater or administrative API added by this change; and
...
- **Connection lifetime does not determine authorization lifetime.** Every
  request must acquire authorization for its route and required keys. Losing all
  connections does not invalidate it; reconnects authenticate the cached SPKI.
  Evidence expiration alone does not end reuse or interrupt active streams.
...
- **Repository metadata does not independently expire a boot-bound
  ...
  miss, then rely on the attested keys until classified failure, invalidation,
  eviction, or process exit. Approval withdrawal requires a policy update and
  restart; metadata discovery alone does not apply it. Do not add a repository
  comparison on cache hits or invent an authorization TTL.
