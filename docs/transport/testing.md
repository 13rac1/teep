...
| Expired entries are removed before valid LRU eviction; concurrent callers retain the original generation without re-attestation | `TestAuthorizationEvictionPrefersExpiredEntries` in [eviction tests](../../internal/proxy/authorization_eviction_test.go) |
...
| SEV expiry uses the selected certificate and embedded root, excludes supplied issuer dates, and remains absent after failed cryptographic verification | `TestSEVCertificateValiditySelection`, `TestSEVOnlineValidityRequiresAuthenticatedEvidence` |
