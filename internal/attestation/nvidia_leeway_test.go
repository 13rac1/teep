...
func TestNVIDIAJWTLeeway(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			admission := NVIDIAAdmission(result)
			if tc.want != nil {
				if !admission.expires.IsZero() {
					t.Fatal("invalid JWT supplied admission time claims")
				}
				return
			}
			if !admission.expires.Equal(now.Add(tc.exp + 10*time.Second)) {
				t.Fatal("admission bound does not include exactly 10 seconds of leeway")
			}
			if !result.ExpiresAt.Equal(now.Add(tc.exp)) {
				t.Fatal("diagnostic expiration changed")
			}
			if err := admission.Check(now.Add(tc.nbf - 10*time.Second - time.Nanosecond)); err == nil {
				t.Fatal("JWT became eligible before its not-before claim")
			}
			if err := admission.Check(now); err != nil {
				t.Fatal(err)
			}
			if err := admission.Check(now.Add(tc.exp + 10*time.Second)); err == nil {
				t.Fatal("expired JWT remained eligible for initial publication")
			}
		})
	}
	if requests.Load() != 1 {
