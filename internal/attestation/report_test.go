...
func TestEvalACIComponentRecognition(t *testing.T) {
	t.Run("pass_approved_repo", func(t *testing.T) {
		in := &ReportInput{
			Raw: &RawAttestation{
				BackendFormat:    FormatACI1,
				ACISourceRepoURL: "https://github.com/Dstack-TEE/private-ai-gateway.git",
			},
			SupplyChainPolicy: &SupplyChainPolicy{
				ACISourceRepos: []string{"https://github.com/Dstack-TEE/private-ai-gateway.git"},
			},
		}
		assertSingleFactor(t, evalComponentRecognition(in), Pass)
	})

	t.Run("fail_unknown_repo", func(t *testing.T) {
		in := &ReportInput{
			Raw: &RawAttestation{
				BackendFormat:    FormatACI1,
				ACISourceRepoURL: "https://github.com/evil/repo.git",
			},
			SupplyChainPolicy: &SupplyChainPolicy{
				ACISourceRepos: []string{"https://github.com/Dstack-TEE/private-ai-gateway.git"},
			},
	...
	t.Run("fail_empty_repo", func(t *testing.T) {
		in := &ReportInput{
			Raw: &RawAttestation{
				BackendFormat: FormatACI1,
			},
	...
	t.Run("not_applicable_no_policy", func(t *testing.T) {
		in := &ReportInput{
			Raw: &RawAttestation{BackendFormat: FormatACI1},
		}
		assertSingleFactor(t, evalComponentRecognition(in), NotApplicable)
	})
}
