...
// APIVersion returns the top-level api_version value in body, or "" when the
// body is not valid JSON or carries no api_version. Callers use it to report
// an unsupported version, which Detect cannot distinguish from an absent
// format in its return value.
func APIVersion(body []byte) string {
	var p probe
