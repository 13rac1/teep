...
func TestAPIVersion(t *testing.T) {
	tests := []struct {
		...
		{"aci/1", `{"api_version":"aci/1"}`, "aci/1"},
		{"unsupported version", `{"api_version":"aci/2"}`, "aci/2"},
		{"absent", `{"intel_quote":"aa"}`, ""},
		{"invalid json", `{`, ""},
		{"wrong type", `{"api_version":3}`, ""},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := formatdetect.APIVersion([]byte(tt.body)); got != tt.want {
				t.Errorf("APIVersion(%s) = %q, want %q", tt.body, got, tt.want)
			}
		})
	}
}
