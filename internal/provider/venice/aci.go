...
// aciResponse is the JSON shape returned by Venice's ACI/1 attestation
...
// digests for ACI/1 workloads (see internal/attestation DefaultAllowFail for
// how the resulting unverifiable factors are handled).
...
// downstream_tls_binding and workload_keyset.tls_public_keys). Wiring
// downstream_tls_binding into the live-SPKI comparison machinery is deferred;
// see docs/plans/2026-07-06-gh-issue-113.md.
...
func aciToRaw(ctx context.Context, ar *aciResponse, unknown, missing []string, body []byte) *attestation.RawAttestation {
	...
	return &attestation.RawAttestation{
		...
		// AppName etc. are intentionally left empty so the compose-based
		// supply-chain factors (compose_binding, sigstore_verification,
		// build_transparency_log, provider_signer_recognition,
		// component_signature_recognition) evaluate honestly to Fail rather
		// than being papered over as NotApplicable. See DefaultAllowFail in
		// internal/attestation/report.go for how those factors are waived
		// (non-blocking, but still visible) for venice until ACI/1 provenance
