...
// ---------------------------------------------------------------------------
//
// These mirror the Rust to_canonical_value() methods in
// vendored/private-ai-gateway/src/aci/types.rs. They build any-typed
// trees that the canonical serializer converts to deterministic bytes,
// ensuring correct integer representation (no float64 precision loss).
...
func (e *aciKeysetEpoch) toCanonicalValue() (map[string]any, error) {
	// NotAfter is u64 in the Rust protocol definition. json.Number
	// preserves the exact wire string; parse as uint64 to match Rust.
...
// ---------------------------------------------------------------------------
//
// Matches the Rust implementation in
// vendored/private-ai-gateway/src/aci/canonical.rs.
// Rejects float64 values — ACI defines only integer numerics.
...
//     using identity key (already REPORTDATA-bound to TDX quote).
