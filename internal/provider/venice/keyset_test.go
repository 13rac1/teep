...
// TestVerifyKeysetEndorsement_HappyPath is the missing happy-path coverage:
// generate a real secp256k1 keypair, build a workload keyset whose identity
// key is that keypair's public key, sign the JCS-canonicalized endorsement
// payload with the private key, and confirm verifyKeysetEndorsement (via
// verifySecp256k1) reports EndorsementValid/KeysetDigestMatch/WorkloadIDMatch
// all true. Every other keyset_test.go case up to this point only exercises
// error paths or pure-canonicalization helpers — none actually produces and
// verifies a valid signature end-to-end.
func TestVerifyKeysetEndorsement_HappyPath(t *testing.T) {
	...
	result := verifyKeysetEndorsement(keyset, keysetDigest, workloadID, sigHex, identityKeyHex)
	if result.Err != nil {
