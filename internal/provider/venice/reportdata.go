...
// key)+nonce layout). The ACI/1 JSON response also carries a top-level
// "report_data" / "quote_report_data" field, but that is an informational
// echo of the same bytes, not a separate binding to verify — those fields
// are intentionally not read here. Do not add ACI/1-specific branching to
// this verifier; if ACI/1 ever changes its REPORTDATA layout, that is a new
// format requiring its own verifier registered via multi.Verifier.
