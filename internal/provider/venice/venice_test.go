...
// validACI1JSON is a structurally complete Venice ACI/1 attestation response
// matching the real format. ACI/1 includes both the new nested "attestation"
// block and dstack-compatible top-level fields.
const validACI1JSON = `{
	"api_version": "aci/1",
...
func TestParseACI1_NoComposeFields(t *testing.T) {
	...
	// ACI/1 has no compose manifest — compose-related fields should be empty.
	if raw.AppCompose != "" {
