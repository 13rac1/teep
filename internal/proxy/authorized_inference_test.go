...
func TestAuthorizedEHBPConcurrentHTTP2(t *testing.T) {
	testtls.RunWithFallbackRoot(t, func(t *testing.T, authority *testtls.Authority) {
		...
		value, err := newAuthorization(key, report, hex.EncodeToString(private.PublicKey().Bytes()), true, false)
		if err != nil {
			t.Fatal(err)
		}
...
func TestAuthorizedRejectionPreservesReplacement(t *testing.T) {
	testtls.RunWithFallbackRoot(t, func(t *testing.T, authority *testtls.Authority) {
		...
		candidate, err := newAuthorization(key, report, hex.EncodeToString(oldKey.PublicKey().Bytes()), true, false)
		if err != nil {
			t.Fatal(err)
		}
		replacement, err = newAuthorization(key, report, hex.EncodeToString(newKey.PublicKey().Bytes()), true, false)
		if err != nil {
			t.Fatal(err)
		}
