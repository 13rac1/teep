...
// raw mux with no Host allowlist or security headers, matching prior
// behavior for those unit tests.
...
// registerRoutes wires all HTTP routes onto s.mux. Monitoring/dashboard
// endpoints (/, /events, /metrics, /explore) and the OpenAI-compatible
// /v1/* surface are all registered here with no per-route auth: access
// control is enforced once, for every route, by hostGuardMiddleware (see
// New and hostguard.go), which rejects requests whose Host header is not in
// the loopback/configured-address allowlist. This defeats DNS-rebinding
// attacks that would otherwise let a malicious page reach the proxy over
// what the browser considers a same-origin connection.
...
func (s *Server) doUpstreamRoundtrip(
	...
	for attempt := range maxAttempts {
		...
		// attested tls_key_fp on EVERY response — cache hit or miss. A
		// TLS-binding provider with no attested fingerprint (empty
		// tlsKeyFP: cache entry predating this field, or attestation that
		// failed to produce one) fails closed rather than skipping the
		// check.
	...
	for i, name := range provNames {
		...
		go func(i int, name string, prov *provider.Provider) {
			...
			for _, raw := range models {
				...
				if err != nil {
					// Fail this provider's entire list rather than silently
					// dropping the one malformed/hostile entry (AGENTS.md:
					// reject malformed input entirely). Other providers'
					// listings are unaffected (partial success across
					// providers, as documented on fetchModels/handleModels).
...
// model id. The dashboard and explore templates render model ids into HTML
// (historically into attribute contexts too — see M5's DOM-API rewrite in
// internal/proxy/templates/). Restricting the accepted characters here
// turns "the dashboard can't be attribute-injected by a model id" into a
// data invariant enforced at the proxy boundary, rather than something that
// depends solely on every current and future rendering call site getting
// escaping right.
...
// string, or the id contains characters outside modelIDPattern — a
// malformed or hostile id (e.g. containing '"', '<', '`') must reject the
// model entry loudly rather than let it reach dashboard rendering; see
// fetchModels, which fails the whole provider's list rather than silently
// dropping just this one malformed entry (AGENTS.md: never silently drop
// malformed elements).
