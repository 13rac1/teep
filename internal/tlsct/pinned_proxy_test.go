			...
			t.Run(failure, func(t *testing.T) {
				...
				if failure == "origin_pin" {
					if !IsOriginTrustFailure(err) {
						t.Fatal("origin pin failure must invalidate origin authorization")
					}
				...
				if failure != "origin_pin" && IsOriginTrustFailure(err) {
					t.Fatal("proxy handshake failure must retain origin authorization")
				}
				if attempt := new(InferenceAttempt); attempt.RetryConnectionFailure(req.Context(), err) {
					t.Fatal("proxy or origin handshake failure must not retry inference")
				}
			})
		}
	})
