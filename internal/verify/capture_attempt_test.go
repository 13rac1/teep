...
func TestVerificationCaptureFinalOutcomeReplay(t *testing.T) {
	...
	for _, outcome := range []string{"success", "response failure", "request failure"} {
		t.Run(outcome, func(t *testing.T) {
			...
			state.evidence = capture.WrapRecording(capture.NewReplayTransport([]capture.RecordedEntry{stale}))
			staleRequest, err := http.NewRequestWithContext(t.Context(), stale.Method, stale.URL, http.NoBody)
			if err != nil {
				t.Fatal(err)
			}
			staleResponse, err := state.evidence.RoundTrip(staleRequest)
			if err != nil {
				t.Fatal(err)
			}
