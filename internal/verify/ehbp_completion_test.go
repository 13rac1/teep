...
func TestStandaloneEHBPStreamCompletion(t *testing.T) {
	for _, mode := range []string{"complete", "truncated", "invalid_tag", "named_error", "provider_error", "null_error", "error_only", "malformed_json"} {
		t.Run(mode, func(t *testing.T) {
			...
			case "named_error":
				failure = "event: error\ndata: {}\n\n"
			case "provider_error", "error_only":
				failure = "data: {\"error\":{\"message\":\"private upstream detail\"}}\n\n"
				if mode == "error_only" {
					prefix = ""
				}
			case "null_error":
			...
			body, nonce := encryptCompletionResponse(t, private, encap, [][]byte{[]byte(prefix + failure + "data: [DONE]\n\n"), []byte("\n")})
			switch mode {
			case "truncated":
